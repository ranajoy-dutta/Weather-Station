...
def toDatabase(data):
    try:
        conn = sql.connect('ws_db.db')       # connecting to database
        cur = conn.cursor()                 # setting the cursor
        # SQL query for insertion into table
        cur.execute("INSERT INTO dataTable (humidity,temperature,timestamps,lat,lng) VALUES ('%s','%s','%s','%s','%s')"%(data['humidity'],
                    data['temperature'],data['timestamp'], data['lat'], data['lng']))
...
def al():
    con = sql.connect("ws_db.db")    # creating connection with database
...
def index():
    ...
    return render_template('index_ws.html', rows = rows)   # rendering the HTML page and passing the fetched values
